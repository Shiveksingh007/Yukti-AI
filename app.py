...
from prompt import *
st.set_page_config(
    page_title="Yukti AI-Auto EDA",
    page_icon="logoBG.png",   # emoji or path to an image file
    layout="wide"     # makes it full width
)

# Create columns for logo + title
col1, col2 = st.columns([1, 10])  # give more space to title

with col1:
    st.image("logoBG.png", width=65)

with col2:
    st.markdown(
        "<h1 style='margin:0; padding:0;'>Yukti AI-Auto EDA</h1>",
        unsafe_allow_html=True
    )

st.image('img_title.jpg')
...
if not gemini_api_key:
    st.sidebar.warning('To enable AI Feature, you must pass API-Key')
    ...
    if df is None:
        ...
        with st.spinner("Running AI..."):
            ...
            if gemini_api_key:
                ...
                st.warning('To enable AI Feature, you must pass API-Key')
                response = None
